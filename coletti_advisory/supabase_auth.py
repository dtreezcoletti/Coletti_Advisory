...
def _resolve_principal(session: SupabaseSession) -> Principal:
    ...
    if len(profiles) != 1:
        raise PermissionError("Authenticated user does not have exactly one active Coletti profile")
    profile = profiles[0]
...
def require_principal(*, app_mode: str) -> Principal | None:
    ...
    Demo mode remains synthetic and unauthenticated when Supabase is not configured.
    Production fails closed. Passwords are submitted directly to Supabase Auth and
    are never stored in application session state.
    """
    if not configured():
        if app_mode == "demo":
            return None
